...
bool INT_RS::isBusy(int i) const {
	return static_cast<bool>(slot_busy[i]);
}

int INT_RS::getIndex(max_size_t tag) const {
	for (int i = 0; i < INTEGERRS_CAP; ++i) {
...
max_size_t INT_RS::getOp(int i) const {
	return to_unsigned(slot_op[i]);
}

max_size_t INT_RS::getVj(int i) const {
	return to_unsigned(slot_vj[i]);
}

max_size_t INT_RS::getQj(int i) const {
	return to_unsigned(slot_qj[i]);
}

max_size_t INT_RS::getVk(int i) const {
	return to_unsigned(slot_vk[i]);
}

max_size_t INT_RS::getQk(int i) const {
	return to_unsigned(slot_qk[i]);
}

max_size_t INT_RS::getRobTag(int i) const {
...
inline int intCandidate(const INT_RS &rs, bool withSquash, max_size_t squashTag) {
	int best = -1;
...
bool INT_RS::execCandidateValid() const {
	return intCandidate(*this, static_cast<bool>(squash.valid),
						to_unsigned(squash.tag)) >= 0;
}

max_size_t INT_RS::execCandidateIndex() const {
	int i = intCandidate(*this, static_cast<bool>(squash.valid),
						 to_unsigned(squash.tag));
	return i >= 0 ? static_cast<max_size_t>(i) : 0;
}

max_size_t INT_RS::execCandidateOp() const {
	int i = intCandidate(*this, static_cast<bool>(squash.valid),
						 to_unsigned(squash.tag));
	return i >= 0 ? to_unsigned(slot_op[i]) : 0;
}

max_size_t INT_RS::execCandidateVj() const {
	int i = intCandidate(*this, static_cast<bool>(squash.valid),
						 to_unsigned(squash.tag));
	return i >= 0 ? to_unsigned(slot_vj[i]) : 0;
}

max_size_t INT_RS::execCandidateVk() const {
	int i = intCandidate(*this, static_cast<bool>(squash.valid),
						 to_unsigned(squash.tag));
	return i >= 0 ? to_unsigned(slot_vk[i]) : 0;
}

max_size_t INT_RS::execCandidateTag() const {
	int i = intCandidate(*this, static_cast<bool>(squash.valid),
						 to_unsigned(squash.tag));
...
void INT_RS::work() {
	...
	if (exec.valid && to_unsigned(exec.src) == 0) {
		auto idx = to_unsigned(exec.index);
		if (idx < static_cast<max_size_t>(INTEGERRS_CAP)
			&& slot_busy[idx]) {
			slot_busy[idx] <= 0;
			slot_qj[idx] <= 0;
			slot_qk[idx] <= 0;
